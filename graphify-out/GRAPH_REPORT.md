...
## Summary
- 635 nodes · 862 edges · 43 communities (38 shown, 5 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 95 edges (avg confidence: 0.84)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f4faefa2`
...
1. `Project Progress — Ford Mustang Mach-E 2026 First Responder RAG Portal` - 23 edges
2. `Review Checklist` - 21 edges
3. `Extrication & Post-Crash Handling` - 15 edges
4. `2. Per-Namespace Pinecone Tool Descriptions` - 15 edges
5. `HudSystem` - 14 edges
6. `High Voltage Battery Pack` - 14 edges
7. `Anti-Patterns to Avoid` - 13 edges
8. `check_invariants()` - 12 edges
9. `main()` - 12 edges
10. `First Responder RAG Portal — EV Emergency Response Guides + 360° Training` - 12 edges
...
Cohesion: 0.08
Nodes (36): addMessage(), addTyping(), autoGrow(), errorEl, initChatBindings(), inputEl, messagesEl, parseN8nResponse() (+28 more)
...
Cohesion: 0.09
Nodes (30): chat_panel.html (First Responder GPT), chat_panel.sendMessage, chat_panel.startRecognition (Web Speech), CloudFront distribution E2FCJOSZVLDA5W, Airbag / pretensioner / stored gas inflator, Cable cut location / no-cut HV cables, High-strength / no-cut structural zone, 400V Li-ion HV battery pack (+22 more)
...
Cohesion: 0.06
Nodes (33): Agents Available, Audio Playback, Available Types, Component Template, Core Architecture, Critical Import Rule, Debugging Missing Features, hzdb (Meta Quest Device Tools) (+25 more)
...
Cohesion: 0.07
Nodes (35): Baseline ("before-fix") n8n First-Responder Chatbot Responses, Q1 — vw_shutdown, Q2a — vw_shutdown_dup, Q3 — nissan_hv, Q4 — vw_nocut, Q5 — unidentified, Post-Fix Webhook Capture Results, Q1 — vw_shutdown (+27 more)
...
Cohesion: 0.16
Nodes (19): build_master(), content_type(), encode_variant(), ffprobe_duration(), ABR HLS ladder (high/mid/low), main(), peak_bandwidth(), process_stem() (+11 more)

### Community 26 - "Community 26"
Cohesion: 0.12
Nodes (16): Deploy, Environment, First Responder RAG Portal — EV Emergency Response Guides + 360° Training, graphify, Known open issues, n8n workflow, Pinecone, Project layout (+8 more)
...
Cohesion: 0.12
Nodes (16): 1. Router Agent — System Message, 2. Per-Namespace Pinecone Tool Descriptions, bmw_ix3_2027, cadillac_lyriq_2023, chevrolet_blazer_ev_2024, chevrolet_bolt_ev_2022_2023, chevrolet_equinox_ev_2024, ford_lightning_2026 (+8 more)

### Community 62 - "Community 62"
Cohesion: 0.13
Nodes (14): 2026-04-16 — Initial Commit, 2026-04-20 — Enriched Metadata + Evaluation Framework, 2026-05-04 — Chatbox Added to Gaussian Splatting Tab, 2026-06-24 — Router Model Finalized: Claude Sonnet 4.6 (Audit-Log Closeout), 2026-06-29 — n8n Router Minimal Hardening (maxIterations + prompt de-dup), Change, Current Pinecone State (as of 2026-04-20), Evaluation Framework (+6 more)
...
Cohesion: 0.25
Nodes (8): 2026-05-24 — IWSDK v2 Portal (Meta Immersive Web SDK Migration), Not yet verified, Pain points hit during build, Rollout model, Scope split — what migrated vs what stayed, Verified locally, What was built, Why

### Community 92 - "Community 92"
Cohesion: 0.15
Nodes (14): 2026-06-01 — IWSDK v2 Live IWER Emulator Pass + Quest STT Correction + Transcribe Webhook, 2026-06-01 — IWSDK v2 VR Interface Overhaul (Follower HUD, Audio/Haptics, Pinch-to-Talk), 2026-06-08 — Streamlit Cut Over to v2 + Desktop Drag-to-Look for the 360° Videosphere, Fix — `DesktopLookSystem` (`portal/src/look-controls.ts`, new), Headline fix — body-locked lazy-follow HUD, Live IWER emulator pass (v2 portal), Other improvements, Polish — inline 🚒 favicon (`portal/index.html`) (+6 more)

### Community 106 - "Community 106"
Cohesion: 0.40
Nodes (5): 2026-05-25 — IWSDK v2 Verify Pass + HUD Comfort Bump + Voice STT Fallback + First v2 Deploy, CORS fix note, HUD comfort height (`portal/src/index.ts`), IWER emulator verify pass (v2 portal), Voice STT fallback (`portal/src/voice.ts`)

### Community 107 - "Community 107"
Cohesion: 0.29
Nodes (7): 2026-06-18 — Ingestion/n8n Audit: Real Bug Was 3 Missing Tool Nodes (Not Re-Ingestion) + `Ford Mache-E` → `vehicle_docs`, Audit findings — the "Mach-E only" theory is FALSE, BLOCKED — live chat verification (expired OpenAI key, NOT our change), Context — the professor's report, Repo cleanup — `Ford Mache-E/` → `vehicle_docs/`, The fix (n8n side — applied + verified structurally), The real bug — 3 missing n8n tool nodes

### Community 108 - "Community 108"
Cohesion: 0.29
Nodes (7): 2026-06-22 — Router Re-Diagnosis: Real Bug Was Retrieval Depth + Prompt Contradiction + Weak Model (Supersedes the 06-18 "3 Missing Tools" Theory) + Eval Set 60→90 + Skill Cleanup, Context — the professor's report, take two, Eval set expanded 60 → 90 (live-class questions), Repo / skill cleanup, The fix (applied via the n8n public API — GET → mutate → PUT), The real root causes (all in the live workflow, none in the repo data), What the re-audit ruled OUT (the assumed causes were wrong)
...
Cohesion: 0.25
Nodes (8): 2026-06-01 — Transcribe Webhook Activated + v2 Overhaul DEPLOYED (the stale-bundle fix), 2026-06-29 — v2 Portal Copy Cleanup + Streamlit VR Caption Reword (Deployed), Caught: the v2 VR overhaul had never been deployed (root cause of "no changes in VR"), Deploy, Note on what does NOT need a deploy, `portal/index.html` (v2), `streamlit_app.py` (VR caption), Transcribe webhook activated + verified end-to-end

### Community 198 - "Community 198"
Cohesion: 0.33
Nodes (9): 2026-05-04 — CloudFront Migration + Streamlit CORS Fix, 2026-05-04 — Meta Quest 3 VR Support (A-Frame Migration), 2026-05-24 — In-VR HUD + Right-Trigger Voice Chat (Meta Quest 3), Changes, Deploy protocol followed, Problem, Solution, Streamlit note (+1 more)
...
## Knowledge Gaps
- **305 isolated node(s):** `npx`, `@meta-quest/hzdb`, `name`, `version`, `private` (+300 more)
...
- **Why does `2. Per-Namespace Pinecone Tool Descriptions` connect `Community 38` to `EV Response Guides & Concepts`?**
